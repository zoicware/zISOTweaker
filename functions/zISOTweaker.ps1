...
if ($Global:config.Count -gt 0) {
    ...
    if (Test-Path "$($info.tempDir)\sources\install.wim") {
        Mount-EditionIfNeeded -ImagePath "$($info.tempDir)\sources\install.wim" -workingDir $info.removeDir -index $info.editionIndex -edition $info.edition
    }
    else {
        Mount-EditionIfNeeded -ImagePath "$($info.tempDir)\sources\install.swm" -workingDir $info.removeDir -index $info.editionIndex -edition $info.edition
    }
